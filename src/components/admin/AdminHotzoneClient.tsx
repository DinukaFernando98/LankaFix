          ...
          {data.total_complaints === 0 ? (
            ...
            <>
              <div className="grid grid-cols-3 gap-4 mb-5">
                {[
                  { label: 'Complaints Mapped', value: data.total_complaints, icon: <MapPin className="w-4 h-4" />, color: 'text-green-600', bg: 'bg-green-50' },
                  { label: 'Clusters (K)', value: data.k_used, icon: <TrendingUp className="w-4 h-4" />, color: 'text-blue-600', bg: 'bg-blue-50' },
                  { label: 'Critical Zones', value: data.clusters.filter((c) => c.priority === 'Critical').length, icon: <Zap className="w-4 h-4" />, color: 'text-red-600', bg: 'bg-red-50' },
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
                <div className="h-[460px]">
                  <HotzoneMap clusters={data.clusters} heatmapPoints={data.heatmap_points} />
                </div>
              ...
              <h2 className="text-sm font-semibold text-gray-900 mb-4">Cluster Breakdown</h2>
              <motion.div
              ...
              >
                {data.clusters.map((cluster) => {
                  ...
                  return (
                    <motion.div
                      key={cluster.cluster_id} variants={item}
                    ...
                    >
                      ...
                      <p className="text-3xl font-bold text-gray-900 mb-0.5">{cluster.complaint_count}</p>
                      <p className="text-xs text-gray-500 mb-3">complaints in this zone</p>
                      <div className="text-xs text-gray-600 space-y-1">
                        <p><span className="text-gray-400">Top issue:</span> {cluster.top_category}</p>
                        <p><span className="text-gray-400">Centre:</span> {cluster.centroid_lat.toFixed(4)}, {cluster.centroid_lng.toFixed(4)}</p>
                      </div>
                    </motion.div>
                  )
                })}
